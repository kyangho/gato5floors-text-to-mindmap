...
function stringify(model, attributes, options) {
  ...
  forEach(this.constructor.jsonColumns, (column) => {
...
function parse(model, response, options = {}) {
  ...
  if (
    !options.parseJsonColumns &&
    options.query &&
    options.query._method !== 'select'
  ) {
    return;
  }
...
module.exports = (Bookshelf) => {
  ...
  const parseOnFetch =
    client === 'sqlite' || client === 'sqlite3' || client === 'mysql';
  ...
  Bookshelf.Model = Bookshelf.Model.extend({
    ...
    save(key, value, options) {
      ...
      Object.keys(attributes).forEach((attribute) => {
        if (
          this.constructor.jsonColumns.includes(attribute) &&
